...
var selectOptions=$dropdown.find("li"); if(!multi){var $selected;if($ul.find(":selected").length){$selected=$ul.find(":selected").last();}
else{$selected=$ul.find("li").first();}
methods._select($dropdown,$selected);}else{methods._select($dropdown,$ul.find(":selected"));} 
$input.addClass($select[0].className); $select.hide().attr("data-dropdownjs",true); $select.after($dropdown); if(options.callback){options.callback($dropdown);} 
...
if(e.which===32&&!$(e.target).is("input")){methods._select($dropdown,$(this));return false;}});$ul.on("focus","li:not(.dropdownjs-add)",function(){if($select.is(":disabled")){return;}
...
$(this).each(function(){initElement($(this));});},select:function(target){var $target=$(this).find("[value=\""+target+"\"]");methods._select($(this),$target);},_select:function($dropdown,$target){if($target.is(".dropdownjs-add"))return;  var $select=$dropdown.data("select"),$input=$dropdown.find("input.fakeinput");var multi=$select.attr("multiple"); var selectOptions=$dropdown.find("li"); if(multi){ $target.toggleClass("selected"); var $selected=$select.find("[value=\""+$target.attr("value")+"\"]");if($selected.prop("selected")){$selected.prop("selected",true);}else{$selected.prop("selected",false);} 
var text=[];selectOptions.each(function(){if($(this).hasClass("selected")){text.push($(this).text());}});$input.val(text.join(", "));} 
if(!multi){ selectOptions.not($target).removeClass("selected"); $target.addClass("selected"); $select.val($target.attr("value")); $input.val($target.text());} 
if($.material){if($input.val().trim()){$select.removeClass("empty");}else{$select.addClass("empty");}}},_addOption:function($ul,$this){ var $option=$("<li></li>"); $option.addClass(this.options.optionStyle); if($this.text()){$option.text($this.text());} 
else{$option.html("&nbsp;");} 
...
if($this.prop("selected")){$option.attr("selected",true);} 
